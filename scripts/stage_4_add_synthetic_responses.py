...
def try_to_restart(
    ...
    try:
        ...
        try:
            ...
            raise
        metadata_persisted["stage_4_start_timestamp"] = datetime.now(UTC).timestamp()
        metadata_persisted["stage_4_start_time_readable"] = datetime.now(UTC).strftime(
            "%Y/%m/%d_%H:%M:%S"
        )
...
if __name__ == "__main__":
    ...
    if args.restart:
        ...
        try:
            ...
            raise
        METADATA["stage_4_start_timestamp"] = datetime.now(UTC).timestamp()
        METADATA["stage_4_start_time_readable"] = datetime.now(UTC).strftime(
            "%Y/%m/%d_%H:%M:%S"
        )
        METADATA["batch_size"] = args.batch_size
        METADATA["stage4_classification_llm_model"] = MODEL_NAME
        METADATA["stage4_classification_llm_location"] = MODEL_LOCATION
