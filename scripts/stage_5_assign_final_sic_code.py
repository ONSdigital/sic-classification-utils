...
def try_to_restart(
    ...
    try:
        ...
        try:
            ...
            raise
        metadata_persisted["stage_5_start_timestamp"] = datetime.now(UTC).timestamp()
        metadata_persisted["stage_5_start_time_readable"] = datetime.now(UTC).strftime(
            "%Y/%m/%d_%H:%M:%S"
        )
...
if __name__ == "__main__":
    ...
    if args.restart:
        ...
        try:
            ...
            raise
        METADATA["stage_5_start_timestamp"] = datetime.now(UTC).timestamp()
        METADATA["stage_5_start_time_readable"] = datetime.now(UTC).strftime(
            "%Y/%m/%d_%H:%M:%S"
        )
